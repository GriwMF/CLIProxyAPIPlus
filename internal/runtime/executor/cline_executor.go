...
// HttpRequest injects provider credentials and executes the supplied HTTP request.
...
func (e *ClineExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			// Re-add "data: " prefix for translator
			lineWithPrefix := append([]byte("data: "), payload...)
			chunks := sdktranslator.TranslateStream(ctx, to, from, req.Model, opts.OriginalRequest, body, bytes.Clone(lineWithPrefix), &param)
			for i := range chunks {
...
func (e *ClineExecutor) Refresh(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	if refreshToken == "" {
		// API key auth or no refresh token — nothing to refresh
		return auth, nil
	}
